...
const categories = ["massage", "facial", "hair", "nails", "barber", "skin", "hair_removal", "lashes_brows", "pamper_package"];

const emptyService: Partial<TablesInsert<"services">> = {
  title: "", description: "", category: "massage", image_url: "", sort_order: 0, is_active: true,
};
...
const AdminServices = () => {
  ...
  const { data: options } = useQuery({
    queryKey: ["admin-service-options", optionsDialog],
    enabled: !!optionsDialog,
    queryFn: async () => {
      const { data, error } = await supabase.from("service_options").select("*").eq("service_id", optionsDialog!).order("sort_order");
      if (error) throw error;
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editing) {
        ...
        const { error } = await supabase.from("services").insert(form as TablesInsert<"services">);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-services"] });
      toast.success(editing ? "Service diupdate!" : "Service ditambahkan!");
      setDialogOpen(false);
      setEditing(null);
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
            ...
            <Card key={s.id}>
              <CardContent className="flex items-center gap-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{s.category} • {s.is_active ? "Aktif" : "Nonaktif"}</p>
                </div>
                <div className="flex gap-1">
                  <Button variant="ghost" size="icon" onClick={() => setOptionsDialog(s.id)}>
          ...
          {services?.length === 0 && <p className="text-center text-muted-foreground py-10">Belum ada service.</p>}
        </div>
      )}
      ...
      <Dialog open={!!optionsDialog} onOpenChange={(o) => { if (!o) setOptionsDialog(null); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={(e) => { e.preventDefault(); addOptionMutation.mutate(); }} className="space-y-3 pt-4 border-t">
            <Input placeholder="Nama (cth: 1 Hour)" value={optionForm.name} onChange={(e) => setOptionForm({ ...optionForm, name: e.target.value })} required />
